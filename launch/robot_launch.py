...
def generate_launch_description():
    ...
    # 寻找 webots 描述的 urdf
    robot_description = pathlib.Path(os.path.join(package_dir, 'resource', 'webots_ur5_description.urdf')).read_text()
    # 寻找 ros2_control
    ...
    # 判断当前系统是否为 win（即nt）来添加前缀
    # 这里系统为 linux（即posix），所以前缀名为空
    controller_manager_prefix = 'python.exe' if os.name == 'nt' else ''
    # 判断环境变量 ROS_DISTRO 是否存在且为 foxy，即判断当前 ROS2 版本是否为 foxy
    use_deprecated_spawner_py = 'ROS_DISTRO' in os.environ and os.environ['ROS_DISTRO'] == 'foxy'

    # ur关节轨迹控制节点
    trajectory_controller_spawner = Node(
        package='controller_manager',
        # 这里 use_deprecated_spawner_py 为假，所以这里 executable='spawner'
